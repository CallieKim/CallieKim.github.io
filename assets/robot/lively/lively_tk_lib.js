...
* @returns {any}
*/
export function solve(solver, goals, weights, time, shape_updates) {
    try {
        _assertClass(solver, Solver);
        const ret = wasm.solve(solver.ptr, addBorrowedObject(goals), addBorrowedObject(weights), time, addBorrowedObject(shape_updates));
...
* @param {any} weights
*/
export function reset(solver, state, weights) {
    try {
        _assertClass(solver, Solver);
        wasm.reset(solver.ptr, addBorrowedObject(state), addBorrowedObject(weights));
    } finally {
        heap[stack_pointer++] = undefined;
        heap[stack_pointer++] = undefined;
    }
...
export class Solver {
    ...
    * @param {boolean | undefined} only_core
    * @param {number | undefined} max_retries
    * @param {number | undefined} max_iterations
    */
    constructor(urdf, objectives, root_bounds, shapes, initial_state, only_core, max_retries, max_iterations) {
        try {
            const ptr0 = passStringToWasm0(urdf, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len0 = WASM_VECTOR_LEN;
            const ret = wasm.solver_new(ptr0, len0, addBorrowedObject(objectives), addBorrowedObject(root_bounds), addBorrowedObject(shapes), addBorrowedObject(initial_state), isLikeNone(only_core) ? 0xFFFFFF : only_core ? 1 : 0, !isLikeNone(max_retries), isLikeNone(max_retries) ? 0 : max_retries, !isLikeNone(max_iterations), isLikeNone(max_iterations) ? 0 : max_iterations);
    ...
    */
    get objectives() {
        const ret = wasm.solver_objectives(this.ptr);
        return takeObject(ret);
    }
    /**
    */
    get currentState() {
        const ret = wasm.solver_current_state(this.ptr);
        return takeObject(ret);
    }
    /**
    */
    get currentGoals() {
        const ret = wasm.solver_current_goals(this.ptr);
        return takeObject(ret);
    }
    /**
    */
    get links() {
        const ret = wasm.solver_links(this.ptr);
        return takeObject(ret);
    }
    /**
    */
    get joints() {
        const ret = wasm.solver_joints(this.ptr);
        return takeObject(ret);
    }
    /**
    * @param {any} state
    ...
    */
    reset(state, weights) {
        try {
            wasm.solver_reset(this.ptr, addBorrowedObject(state), addBorrowedObject(weights));
        } finally {
            heap[stack_pointer++] = undefined;
            heap[stack_pointer++] = undefined;
        }
    ...
    */
    solve(goals, weights, time, shape_updates) {
        try {
            const ret = wasm.solver_solve(this.ptr, addBorrowedObject(goals), addBorrowedObject(weights), time, addBorrowedObject(shape_updates));
            return takeObject(ret);
        } finally {
            heap[stack_pointer++] = undefined;
            heap[stack_pointer++] = undefined;
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('lively_tk_lib_bg.wasm', import.meta.url);
    }
    const imports = getImports();
